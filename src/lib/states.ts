 ...
 * GST state codes.
 *
 * Lifted from `obez-erp-web/src/lib/data/pincode.ts` so the API and the browser
 * agree on what a state is. The first two digits of a GSTIN are this code, and
 * a site's code is what `derivePlaceOfSupply` compares — so a disagreement here
 * is a wrong tax head, not a cosmetic difference.
 */
